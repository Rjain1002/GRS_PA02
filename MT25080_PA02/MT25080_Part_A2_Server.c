...
void *handle_client(void *arg) {
  ...
  // 3. Prepare iovec (Scatter-Gather) - ELIMINATES USER-SIDE COPY
  ...
  while (time(NULL) - start_time < hs.duration_seconds) {
    // COPY 1 REMOVED.
    // COPY 2: User-to-Kernel (sendmsg syscall copies from iov to kernel)
    ssize_t sent = sendmsg(client_fd, &msgh, 0);
    if (sent <= 0)
      break;
